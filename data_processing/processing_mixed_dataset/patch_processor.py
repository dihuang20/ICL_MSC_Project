...
# remove patch seperate symbols in commit patch
def remove_patch_seperate_symbol(commit_patch):
    lines_list = commit_patch.split('\n')
    new_lines_list = []
    for line in lines_list:
        if line == '-----<485_PATCH_SEP_217>-----':
            continue
        new_lines_list.append(line)
    return '\n'.join(new_lines_list)

# check if commit patch only includes comment
def check_if_only_comment(commit_patch, commit_patch_without_comment):
    lines_list = commit_patch_without_comment.split('\n')
    for line in lines_list:
        if line.startswith('+') or line.startswith('-'):
            line = line.strip()
            if len(line) > 1:
                return False
    return len(commit_patch_without_comment) < len(commit_patch)
...
def preprocess_single_commit_patch(config, commit_patch):
    ...
    # remove patch seperate symbols
    commit_patch = remove_patch_seperate_symbol(commit_patch)
    ...
    # check if only comment
    commit_patch_without_comment = comment_remover(commit_patch)
    if check_if_only_comment(commit_patch, commit_patch_without_comment):
        return ONLY_COMMENT_COMMIT_PATCH

    # remove comments
    if removing_comments:
        commit_patch = commit_patch_without_comment
